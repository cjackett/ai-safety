...
MIN_LABEL_PERCENT = 3
MAX_SINGLE_ROW_MODELS = 3

StatsDict = dict[str, int]
ModelStats = dict[str, StatsDict]
AttackStats = dict[str, StatsDict]
CategoryStats = dict[str, dict[str, StatsDict]]
...
def load_results(config: dict[str, Any], results_file: Path | None = None) -> list[dict[str, Any]]:
    """Load multimodal evaluation results from JSON file."""
    base_dir = Path(__file__).parent.resolve()

    if results_file is None:
        # Find most recent results file in raw/ subdirectory
        results_dir = base_dir / config["paths"]["raw_results"]
        json_files = list(results_dir.glob("multimodal_safety*.json"))

        if not json_files:
            msg = f"No results files found in {results_dir}. Run run_multimodal_safety.py first to generate results."
...
def compute_compliance_stats(results: list[dict[str, Any]]) -> ModelStats:
    """Compute compliance statistics by model."""
    model_stats: ModelStats = defaultdict(
        lambda: {
            "total": 0,
            "refused": 0,
            "hedged": 0,
...
def compute_attack_type_stats(results: list[dict[str, Any]]) -> AttackStats:
    """Compute success rates by attack type."""
    attack_stats: AttackStats = defaultdict(
        lambda: {
            "total": 0,
            "refused": 0,
            "hedged": 0,
...
def compute_category_stats(results: list[dict[str, Any]]) -> CategoryStats:
    """Compute compliance statistics by category and model."""
    category_stats: CategoryStats = defaultdict(
        lambda: defaultdict(
            lambda: {
                "refused": 0,
                "hedged": 0,
...
def plot_compliance_distribution(model_stats: ModelStats, output_dir: Path) -> None:
    ...
    # Stack bars
    ax.bar(x, refused, width, label="Refused", color="#2ecc71")
    ax.bar(x, hedged, width, bottom=refused, label="Hedged", color="#f39c12")
    ax.bar(x, full, width, bottom=np.array(refused) + np.array(hedged), label="Full Compliance", color="#e74c3c")
    ...
    for i, model in enumerate(models):
        ...
        if total > 0:
            y_offset = 0.0
            for value in [refused[i], hedged[i], full[i]]:
    ...
    plt.tight_layout()
    output_file = output_dir / "compliance_distribution.png"
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
    plt.close()

    logger.info(f"Saved compliance_distribution.png to {output_dir}")


def plot_attack_effectiveness(attack_stats: AttackStats, output_dir: Path) -> None:
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([a.replace("_", " ").title() for a in attack_types], rotation=45, ha="right")
    ax.set_ylim(0, max(success_rates) + 10 if success_rates else 10)

    # Add value labels
    for bar, rate in zip(bars, success_rates, strict=False):
        if rate > 0:
            ax.text(bar.get_x() + bar.get_width() / 2.0, rate + 1, f"{rate:.1f}%", ha="center", va="bottom", fontsize=9)

    plt.tight_layout()
    output_file = output_dir / "attack_effectiveness.png"
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
    plt.close()

    logger.info(f"Saved attack_effectiveness.png to {output_dir}")


def plot_radar_charts(results: list[dict[str, Any]], output_dir: Path) -> None:  # noqa: PLR0915
    ...
    models = sorted({r["model"] for r in results if not r.get("error")})
    categories = sorted({r["category"] for r in results if not r.get("error")})
    ...
    if num_models <= MAX_SINGLE_ROW_MODELS:
        fig, axes = plt.subplots(1, num_models, figsize=(6 * num_models, 6), subplot_kw={"projection": "polar"})
        if num_models == 1:
    ...
    for idx, model in enumerate(models):
        ...
        for category in categories:
            subset = [r for r in results if r["model"] == model and r["category"] == category and not r.get("error")]
            total = len(subset)
        ...
        # Add edge lines for clarity with larger markers
        ax.plot(angles, refused_outer, "o-", linewidth=1.5, color="#27ae60", alpha=0.8, markersize=6)
        ax.plot(angles, hedged_outer, "o-", linewidth=1.5, color="#e67e22", alpha=0.8, markersize=6)

        # Add refusal rate labels at each category point for clarity
        for i, (angle, rate) in enumerate(zip(angles[:-1], refused_rates[:-1], strict=False)):
            # Position label slightly outside the data point
            label_radius = max(rate + 8, 15)  # Ensure minimum distance from center
            ax.text(
                angle,
                label_radius,
                f"{rate:.0f}%",
        ...
        ax.text(0.5, 1.17, model_name, transform=ax.transAxes, ha="center", va="top", fontsize=12, fontweight="bold")
    ...
    if num_models <= MAX_SINGLE_ROW_MODELS:
        ...
        plt.suptitle(
            "Multimodal Safety Performance: Category-Specific Compliance Distribution\n"
    ...
    logger.info(f"Saved radar_charts.png to {output_dir}")


def generate_statistics_report(  # noqa: C901, PLR0912, PLR0915
    ...
    logger.info(f"Generating statistics report: {report_file}")
    ...
    with report_file.open("w") as f:
        ...
        ocr_verified = len([r for r in results if r.get("ocr_verified") is True])
        ocr_failed = len([r for r in results if r.get("ocr_verified") is False])

        if total_tests_with_ocr > 0:
            f.write(
                f"- OCR verified: {ocr_verified}/{total_tests_with_ocr} "
                f"({ocr_verified/total_tests_with_ocr*100:.1f}%)\n",
            )
            f.write(
                f"- OCR failed: {ocr_failed}/{total_tests_with_ocr} "
                f"({ocr_failed/total_tests_with_ocr*100:.1f}%)\n\n",
            )

            # OCR failure by attack type
            attack_ocr_stats: dict[str, dict[str, int]] = defaultdict(lambda: {"verified": 0, "failed": 0})
            for r in results:
            ...
            model_ocr_stats: dict[str, dict[str, int]] = defaultdict(lambda: {"verified": 0, "failed": 0})
            for r in results:
                if r.get("error") or r.get("ocr_verified") is None:
                    continue
            ...
            for model in sorted(model_ocr_stats.keys()):
                ...
                if total > 0:
                    model_name = model.replace("ollama/", "")
                    f.write(
                        f"- {model_name}: {stats['verified']}/{total} verified ({stats['verified']/total*100:.1f}%)\n",
                    )
            f.write("\n")
        ...
        repetitive_responses = [r for r in results if r.get("response_repetitive") is True]
        total_tests_checked = len([r for r in results if "response_repetitive" in r and not r.get("error")])

        if total_tests_checked > 0:
            f.write(
                f"- Repetitive responses detected: {len(repetitive_responses)}/{total_tests_checked} "
                f"({len(repetitive_responses)/total_tests_checked*100:.1f}%)\n\n",
            )

            if repetitive_responses:
                f.write("**Repetitive responses by model:**\n")
                model_rep_count: dict[str, int] = defaultdict(int)
                ...
                attack_rep_count: dict[str, int] = defaultdict(int)
                for r in repetitive_responses:
    ...
    logger.info(f"Saved statistics_report.md to {output_dir}")


def main() -> None:
    """Run multimodal results analysis."""
    parser = argparse.ArgumentParser(description="Analyse multimodal evaluation results and generate visualizations")
    parser.add_argument(
        "results_file",
        nargs="?",
        type=Path,
    ...
    # Load results
    results = load_results(config, args.results_file)
    ...
    logger.info(f"  - Safety tests (harmful): {len(safety_results)}")
    logger.info(f"  - Benign control tests: {benign_count}")
    ...
    figures_dir = base_dir / config["paths"]["figures"]
    analysis_dir = base_dir / config["paths"]["analysis"]
    figures_dir.mkdir(parents=True, exist_ok=True)
    analysis_dir.mkdir(parents=True, exist_ok=True)

    logger.info("Generating visualizations...")

    # Generate plots (using filtered safety results) - save to figures/
    plot_compliance_distribution(model_stats, figures_dir)
    plot_attack_effectiveness(attack_stats, figures_dir)
    plot_radar_charts(safety_results, figures_dir)

    # Generate statistics report (using all results for benign control reporting) - save to analysis/
    generate_statistics_report(results, model_stats, attack_stats, category_stats, analysis_dir)
    ...
    logger.info("  Figures:")
    logger.info("    - compliance_distribution.png")
    logger.info("    - attack_effectiveness.png")
