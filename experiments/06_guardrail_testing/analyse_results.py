...
def plot_jailbreak_effectiveness(summaries: dict[str, dict[str, Any]], output_dir: Path) -> None:
    ...
    ax.bar(x, input_blocks, width, label="Blocked at Input", color="#2ecc71")
    ax.bar(x, output_blocks, width, bottom=input_blocks, label="Blocked at Output", color="#f39c12")
    ax.bar(
        x,
        bypasses,
        width,
        bottom=np.array(input_blocks) + np.array(output_blocks),
        label="Bypassed",
        color="#e74c3c",
    )
    ...
    for i in range(len(configs)):
        ...
        if input_blocks[i] > LABEL_MIN_PCT:
            ax.text(
                i,
                input_blocks[i] / 2,
                f"{input_blocks[i]:.1f}%",
        ...
        if output_blocks[i] > LABEL_MIN_PCT:
            y_pos = input_blocks[i] + output_blocks[i] / 2
            ax.text(
                i,
                y_pos,
                f"{output_blocks[i]:.1f}%",
                ha="center",
                va="center",
                fontweight="bold",
                color="white",
    ...
    logger.info("Saved: jailbreak_effectiveness.png")
    plt.close()


def plot_latency_comparison(summaries: dict[str, dict[str, Any]], output_dir: Path) -> None:
...
def plot_security_vs_usability(summaries: dict[str, dict[str, Any]], output_dir: Path) -> None:
    ...
    logger.info("Saved: security_vs_usability.png")
    plt.close()


def plot_attack_category_heatmap(summaries: dict[str, dict[str, Any]], output_dir: Path) -> None:
        ...
        for config in configs:
            if "jailbreak_resistance" in summaries[config]:
                ...
                if "by_category" in jr and category in jr["by_category"]:
                    ...
                    block_rate = (
                        ((stats["blocked_at_input"] + stats["blocked_at_output"]) / stats["total"] * 100)
                        if stats["total"] > 0
                        else 0
                    )
    ...
    logger.info("Saved: attack_category_heatmap.png")
    plt.close()


def plot_radar_charts(  # noqa: C901, PLR0912, PLR0915
    ...
    fig, axes = plt.subplots(1, num_configs, figsize=(6 * num_configs, 6), subplot_kw={"projection": "polar"})
    if num_configs == 1:
    ...
    for idx, config in enumerate(configs):
        ...
        output_outer = [(i + o) for i, o in zip(input_rates, output_rates, strict=False)]
        bypass_outer = [100] * len(angles)

        # Plot stacked areas from inside to outside
        ax.fill_between(angles, center, input_outer, alpha=0.7, color=colors["input"], label="Input Blocked")
        ax.fill_between(angles, input_outer, output_outer, alpha=0.7, color=colors["output"], label="Output Blocked")
        ax.fill_between(angles, output_outer, bypass_outer, alpha=0.7, color=colors["bypassed"], label="Bypassed")
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, -0.02), ncol=3, frameon=True, fontsize=11)
...
def generate_markdown_report(summaries: dict[str, dict[str, Any]], output_dir: Path) -> None:
...
def main() -> None:
    ...
    # Load all summary files from analysis/ subdirectory
    summary_files = list(analysis_dir.glob("summary_*.json"))

    if not summary_files:
        logger.error(f"No summary files found in {analysis_dir}")
        return

    logger.info("=" * 60)
    logger.info("Analysing Guardrail Test Results")
    logger.info(f"Found {len(summary_files)} configuration(s)")
    logger.info("=" * 60)
    ...
    # Create visualizations (save to figures/ subdirectory)
    logger.info("Generating visualizations...")

    plot_jailbreak_effectiveness(summaries, figures_dir)
    plot_latency_comparison(summaries, figures_dir)
    plot_security_vs_usability(summaries, figures_dir)
    plot_attack_category_heatmap(summaries, figures_dir)
    plot_radar_charts(summaries, figures_dir)

    # Generate report (save to analysis/ subdirectory)
    generate_markdown_report(summaries, analysis_dir)
